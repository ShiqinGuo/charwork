...
**日期**: 2026-04-21  
**范围**: 提交附件评价、学生主动 AI 总评、评价可见性、异步任务编排、前端提交详情改造与旧接口清理  
**目标**: 将当前提交级 `ai_feedback` 重构为以 AI 反馈为核心的独立模型，支持附件级评价、学生主动总评，并同步完成前端改造与旧接口/旧方法删除
...
- 为未来扩展不同反馈对象预留统一模型
- 学生端提交详情页改为“附件逐项评价 + 我的 AI 总评”同页双区块
- 删除旧的提交级 AI 评语接口、前端 API 方法与 student-class 相关旧入口，不保留兼容层
...
- `submission.ai_feedback`
  - 不再作为新功能主存储
  - 代码层彻底停用
  - 所有新接口与新前端页面均不得再读取该字段
  - 字段本身是否在数据库中删除，可在功能稳定后通过独立迁移处理
...
- 提交域中任何基于 `submission.ai_feedback` 聚合返回的旧 service 方法
- 前端旧 `getAIFeedback` 调用
- student-class 域中任何继续指向旧提交级 AI 评语语义的入口
...
- 学生端提交详情页采用同页双区块：
  - 附件逐项评价区
  - 我的 AI 总评区
- 教师端批改页删除旧整次提交 AI 评语展示，只显示附件逐项评价
- 学生作业列表页不再展示旧提交级 AI 评语摘要
- 删除 `student-class` 相关旧 AI 评语入口或改造为新接口体系，不保留重复语义路径

这样可以先稳定后端数据链路，再一次性完成前端切换和旧入口清理，避免长时间双轨并存。
...
- 旧 `submission.ai_feedback` 不再被新链路写入，也不再被新链路读取
- 前端不再调用旧 `ai-feedback` 接口
- student-class 旧 AI 评语入口已删除或完成迁移
...
### 为什么这次不保留旧接口兼容层
- 兼容层会让前端和 student-class 继续持有旧语义入口
- 双轨接口会增加测试和维护成本
- 你的目标是同步完成前端改造并彻底删除旧接口，因此应一次性收口
