...
数据流：
1. 配置启用的业务表 `assignment/comment/hanzi/course/teaching_class/student` 发生变更
2. 共享字典表 `hanzi_dictionary` 走独立字典索引同步
3. Canal 产出 insert/update/delete 事件并投递 RabbitMQ
4. Worker 根据配置白名单和注册表分发到业务检索或字典检索服务
5. ES upsert/delete 完成增量同步
...
- 幂等策略：ES 文档 ID 固定为 `{table}_{id}`，重复消费仅覆盖
...
- `isDdl == false`
- 若 `table` 已配置但未在业务检索注册表中定义，Worker 仅记录 warning，不做写入
...
- 配置变更后必须重启 API 与 Search Sync Worker，自启动配置也要同步更新
- 监控项：
  - 消费速率、积压长度、失败数
  - ES 写入耗时与失败率
  - 重试与死信队列长度
...
- 极端情况下可调用 `/api/v1/search/reindex` 做业务索引全量补齐
