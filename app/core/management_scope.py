...
为多租户场景提供数据隔离支持，通过 Query 参数或 Header 指定管理系统上下文，
或自动回退到用户的默认管理系统。
"""
...
async def get_management_scope(
    ...
    """
    功能描述：
        FastAPI 依赖：获取当前请求的管理系统作用域。
        Query 参数优先于 Header，便于接口调试时显式覆盖客户端默认上下文。
        未显式指定时回退到用户的默认管理系统。

    参数：
        management_system_id (Optional[str]): Query 参数，管理系统 ID。
        x_management_system_id (Optional[str]): Header 参数，管理系统 ID。
        current_user (User): 当前登录用户对象。
        db (AsyncSession): 数据库会话。

    返回值：
        ManagementScope: 管理系统作用域信息。

    异常：
        HTTPException(403): 用户无权访问指定的管理系统。
    """
